...
int main(int argc, char *argv[]) {
	...
	/* argc keeps track of the number of arguments passed to the program i.e., ./pingser 8888 would make argc = 2 */
	if (argc > 1) { 
		/* strncpy(destination, source, # of bytes)
		 *
		 * Grabbing the port # from when the user calls the program, i.e., pulling 8888 when the user calls
		 * pingser. Allocating 32-bits of memory 
		 */
		strncpy(portnum, argv[1], 5);
		portnum[5] = '\0';
	} else {
		/* If the user does not input any value for the program, default to 8888 */
		strcpy(portnum, "8888");
	}
